...
var DependencyGraphHelpers = function () {
  ...
  _createClass(DependencyGraphHelpers, [{
    ...
    key: 'isProvidesModuleNodeModulesDir',
    value: function isProvidesModuleNodeModulesDir(file) {
      var index = file.lastIndexOf(NODE_MODULES);
      if (index === -1) {
        return false;
      }
    ...
    key: 'isProvidesModuleDir',
    value: function isProvidesModuleDir(file) {
      var index = file.lastIndexOf(NODE_MODULES);
      if (index !== -1) {
        return false;
      }
      ...
      var dirs = this._providesModuleNodeModules;
      for (var i = 0; i < dirs.length; i++) {
        var _index = parts.indexOf(dirs[i]);
        if (_index > -1) {
          var packageJsonFile = path.join(path.join.apply(null, parts.slice(0, _index + 1)), 'package.json');
